...
public class KeywordSuffixScrambler {
	...
	public KeywordSuffixScrambler(
		...
		bijections = new FeistelBijection[MAX_SHORT_CODE_LEN];
		long M = 62L;
		for (int k = 1; k <= 6; k++) {
	...
	public long scramble(long i, String keyword) {
		return bijections[MAX_SHORT_CODE_LEN - keyword.length()].permute(i);
	}

	public long suffixSpace(String keyword) {
		return bijections[MAX_SHORT_CODE_LEN - keyword.length()].getM();
	}
