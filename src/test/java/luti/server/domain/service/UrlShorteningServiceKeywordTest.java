...
class UrlShorteningServiceKeywordTest {
	...
	// -------------------------------------------------------------------------
	// 시나리오 1: keyword 자체로 첫 시도 성공
	// -------------------------------------------------------------------------

	@Nested
	@DisplayName("keyword 자체로 첫 번째 삽입 성공")
	class KeywordDirectInsertSuccess {

		@Test
		@DisplayName("keyword 단독 삽입 성공 - 멤버 없음 (비로그인)")
		void keyword_직접삽입_성공_비로그인() {
			// Given
			...
			System.out.println("=== keyword 직접 삽입 성공 (비로그인) ===");
			System.out.println("keyword: " + keyword);
			System.out.println("originalUrl: " + ORIGINAL_URL);
			...
			assertTrue(result.endsWith(keyword));

			System.out.println("생성된 shortUrl: " + result);

			verify(atomicInserter, times(1)).tryInsert(any(UrlMapping.class));
			verify(counterRedisTemplate, never()).opsForValue();
		}

		@Test
		@DisplayName("keyword 단독 삽입 성공 - 멤버 있음 (로그인)")
		void keyword_직접삽입_성공_로그인() {
			// Given
			String keyword = "abc";
			Long memberId = 1L;
			...
			when(base62Encoder.decode(keyword)).thenReturn(10L);
			when(idScrambler.descramble(10L)).thenReturn(5L);
			when(atomicInserter.tryInsert(any(UrlMapping.class))).thenReturn(true);

			System.out.println("=== keyword 직접 삽입 성공 (로그인) ===");
			System.out.println("keyword: " + keyword);
		...
		@Test
		@DisplayName("keyword 단독 삽입 성공 - 7글자 keyword")
		void keyword_직접삽입_성공_7글자() {
			// Given
			...
			System.out.println("=== 7글자 keyword 직접 삽입 성공 ===");
			...
			verify(atomicInserter, times(1)).tryInsert(any(UrlMapping.class));
			// 7글자이면 첫 tryInsert 성공 시 suffix 로직 진입하지 않음
			verify(counterRedisTemplate, never()).opsForValue();
		}
	}

	// -------------------------------------------------------------------------
	// 시나리오 2: keyword 자체 충돌 → suffix 루프에서 성공
	// -------------------------------------------------------------------------

	@Nested
	@DisplayName("keyword 충돌 후 suffix 루프에서 성공")
	class KeywordCollisionThenSuffixSuccess {

		@Test
		@DisplayName("keyword 충돌 - suffix 첫 번째 시도 성공")
		void keyword_충돌_suffix_첫시도_성공() {
			// Given
			String keyword = "hi"; // 2글자, suffix 공간 있음
			long suffixSpace = 3_844L; // 62^2

			when(base62Encoder.decode(eq(keyword))).thenReturn(100L);
			when(idScrambler.descramble(anyLong())).thenReturn(50L);
			when(keywordSuffixScrambler.suffixSpace(keyword)).thenReturn(suffixSpace);
			when(keywordSuffixScrambler.scramble(0L, keyword)).thenReturn(777L);
			when(base62Encoder.encode(777L)).thenReturn("cX");
			when(base62Encoder.decode(eq("hi" + "cX"))).thenReturn(200L);

			// 첫 번째(keyword 자체) 실패, 두 번째(suffix 포함) 성공
			when(atomicInserter.tryInsert(any(UrlMapping.class)))
				.thenReturn(false)
				.thenReturn(true);

			when(counterRedisTemplate.opsForValue()).thenReturn(valueOperations);
			when(valueOperations.increment("kw:cnt:" + keyword)).thenReturn(1L); // rawCounter=1, N=0

			System.out.println("=== keyword 충돌 후 suffix 루프 첫 시도 성공 ===");
			System.out.println("keyword: " + keyword);
		...
		@Test
		@DisplayName("keyword 충돌 - suffix 루프 두 번째 시도에서 성공")
		void keyword_충돌_suffix_두번째시도_성공() {
			// Given
			...
			// attempt=0: scramble(0, keyword) -> 111, encode(111) -> "1Z"
			when(keywordSuffixScrambler.scramble(0L, keyword)).thenReturn(111L);
			when(base62Encoder.encode(111L)).thenReturn("1Z");
			when(base62Encoder.decode("test1Z")).thenReturn(500L);

			// attempt=1: scramble(1, keyword) -> 222, encode(222) -> "3g"
			when(keywordSuffixScrambler.scramble(1L, keyword)).thenReturn(222L);
			when(base62Encoder.encode(222L)).thenReturn("3g");
			when(base62Encoder.decode("test3g")).thenReturn(600L);

			// keyword 자체 실패, attempt=0 실패, attempt=1 성공
			when(atomicInserter.tryInsert(any(UrlMapping.class)))
				.thenReturn(false)
				.thenReturn(false)
				.thenReturn(true);
			...
			when(valueOperations.increment("kw:cnt:" + keyword))
				.thenReturn(1L) // attempt=0: N=0
				.thenReturn(2L); // attempt=1: N=1

			System.out.println("=== keyword 충돌 - suffix 루프 두 번째 시도 성공 ===");
			System.out.println("keyword: " + keyword);
			...
			verify(atomicInserter, times(3)).tryInsert(any(UrlMapping.class));
	...
	class SevenCharKeywordCollision {
		...
		void keyword_7글자_충돌_예외() {
			...
			// suffix 로직으로 진입하지 않음
			verify(counterRedisTemplate, never()).opsForValue();
			verify(keywordSuffixScrambler, never()).suffixSpace(any());
		}
	...
	class SuffixSpaceExhausted {
		...
		void N이_suffixSpace_이상일때_예외() {
			...
			when(base62Encoder.decode(keyword)).thenReturn(100L);
			when(idScrambler.descramble(anyLong())).thenReturn(50L);
			when(keywordSuffixScrambler.suffixSpace(keyword)).thenReturn(suffixSpace);
			...
			when(counterRedisTemplate.opsForValue()).thenReturn(valueOperations);
			// rawCounter = suffixSpace + 1 → N = suffixSpace >= suffixSpace → 초과
			when(valueOperations.increment("kw:cnt:" + keyword))
			...
			System.out.println("keyword: " + keyword);
			System.out.println("suffixSpace: " + suffixSpace);
			System.out.println("rawCounter: " + (suffixSpace + 1) + " -> N=" + suffixSpace);
		...
		void N이_suffixSpace와_같을때_경계값_예외() {
			...
			when(base62Encoder.decode(keyword)).thenReturn(200L);
			when(idScrambler.descramble(anyLong())).thenReturn(100L);
			when(keywordSuffixScrambler.suffixSpace(keyword)).thenReturn(suffixSpace);

			when(atomicInserter.tryInsert(any(UrlMapping.class))).thenReturn(false);

			when(counterRedisTemplate.opsForValue()).thenReturn(valueOperations);
			// rawCounter = suffixSpace + 1 → N = suffixSpace (경계값, 정확히 초과)
			when(valueOperations.increment("kw:cnt:" + keyword))
				.thenReturn(suffixSpace + 1);
	...
	class MaxRetryExceeded {
		...
		void 최대재시도_초과_예외() {
			...
			when(keywordSuffixScrambler.scramble(0L, keyword)).thenReturn(10L);
			when(keywordSuffixScrambler.scramble(1L, keyword)).thenReturn(20L);
			when(keywordSuffixScrambler.scramble(2L, keyword)).thenReturn(30L);
			when(keywordSuffixScrambler.scramble(3L, keyword)).thenReturn(40L);
			when(keywordSuffixScrambler.scramble(4L, keyword)).thenReturn(50L);
			...
			// keyword 자체 + 5번 retry 모두 삽입 실패 (총 6번)
			when(atomicInserter.tryInsert(any(UrlMapping.class))).thenReturn(false);
			...
			when(valueOperations.increment("kw:cnt:" + keyword))
				.thenReturn(1L)  // attempt 0: N=0
				.thenReturn(2L)  // attempt 1: N=1
				.thenReturn(3L)  // attempt 2: N=2
				.thenReturn(4L)  // attempt 3: N=3
				.thenReturn(5L); // attempt 4: N=4
			...
			// keyword 자체(1번) + 5번 retry = 총 6번 tryInsert 호출
			verify(atomicInserter, times(6)).tryInsert(any(UrlMapping.class));
		...
		void 최대재시도_직전_성공() {
			...
			when(keywordSuffixScrambler.scramble(0L, keyword)).thenReturn(1L);
			when(keywordSuffixScrambler.scramble(1L, keyword)).thenReturn(2L);
			when(keywordSuffixScrambler.scramble(2L, keyword)).thenReturn(3L);
			when(keywordSuffixScrambler.scramble(3L, keyword)).thenReturn(4L);
			when(keywordSuffixScrambler.scramble(4L, keyword)).thenReturn(5L);
			...
			// keyword 자체 실패 + attempt 0~3 실패 + attempt 4 성공
			when(atomicInserter.tryInsert(any(UrlMapping.class)))
				.thenReturn(false)
				.thenReturn(false)
				.thenReturn(false)
	...
	class InvalidKeywordFormat {
		...
		@Test
		@DisplayName("유효한 keyword - 소문자만 (1글자)")
		...
		@Test
		@DisplayName("유효한 keyword - 대소문자 숫자 혼합 (7글자)")
	...
	class ShortUrlFormatVerification {
		...
		@Test
		@DisplayName("keyword 직접 삽입 성공 - shortUrl은 DOMAIN/keyword 형식")
		void shortUrl_형식_검증() {
			// Given
			String keyword = "test";
